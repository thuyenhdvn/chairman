	...
	class TGM_Plugin_Activation {
		...
		protected function add_admin_menu( array $args ) {
			if ( has_filter( 'tgmpa_admin_menu_use_add_theme_page' ) ) {
				_deprecated_function( 'The "tgmpa_admin_menu_use_add_theme_page" filter', '2.5.0', esc_html( 'Set the parent_slug config variable instead.', 'tgmpa' ) );
			}
		...
		protected function do_plugin_install() {
			...
			if ( ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' === $_GET['tgmpa-install'] ) || ( isset( $_GET['tgmpa-update'] ) && 'update-plugin' === $_GET['tgmpa-update'] ) ) {
				...
				if ( $this->is_tgmpa_complete() ) {
					echo '<p>', sprintf( esc_html( $this->strings['complete'] ), '<a href="' . esc_url( self_admin_url() ) . '">' . esc_html( 'Return to the Dashboard', 'tgmpa' ) . '</a>' ), '</p>';
			...
			if ( ! empty( $desired_slug ) ) {
				...
				if ( ! empty( $subdir_name ) && $subdir_name !== $desired_slug ) {
					...
					if ( true === $GLOBALS['wp_filesystem']->move( $from, $to ) ) {
						...
						return new WP_Error( 'rename_failed', esc_html( 'The remote plugin package does not contain a folder with the desired slug and renaming did not work.', 'tgmpa' ) . ' ' . esc_html( 'Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', 'tgmpa' ), array( 'found' => $subdir_name, 'expected' => $desired_slug ) );
					}
				} elseif ( empty( $subdir_name ) ) {
					return new WP_Error( 'packaged_wrong', esc_html( 'The remote plugin package consists of more than one file, but the files are not packaged in a folder.', 'tgmpa' ) . ' ' . esc_html( 'Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', 'tgmpa' ), array( 'found' => $subdir_name, 'expected' => $desired_slug ) );
				}
			}
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		public function no_items() {
			printf( wp_kses_post( wp_kses(__( 'No plugins to install, update or activate. <a href="%1$s">Return to the Dashboard</a>', 'tgmpa' ), array('a'=>array('href'=>array()))) ), esc_url( self_admin_url() ) );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		}
				...
				<tr class="plugin-update-tr">
					<td colspan="', absint( $this->get_column_count() ), '" class="plugin-update colspanchange">
						<div class="update-message">',
							esc_html( 'Upgrade message from the plugin author:', 'tgmpa' ),
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
				...
				if ( empty( $_POST['plugin'] ) ) {
					echo '<div id="message" class="error"><p>', esc_html( 'No plugins were selected to be activated. No action taken.', 'tgmpa' ), '</p></div>';
				...
				if ( empty( $plugins_to_activate ) ) {
					echo '<div id="message" class="error"><p>', esc_html( 'No plugins are available to be activated at this time.', 'tgmpa' ), '</p></div>';
					...
					public function add_strings() {
						if ( 'update' === $this->options['install_type'] ) {
							...
							if ( $this->tgmpa->is_automatic ) {
								...
								$this->upgrader->strings['skin_update_successful']    = esc_html__( '%1$s installed and activated successfully.', 'tgmpa' ) . ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html( 'Show Details', 'tgmpa' ) . '</span><span class="hidden">' . esc_html( 'Hide Details', 'tgmpa' ) . '</span>.</a>';
								...
								$this->upgrader->strings['skin_update_successful']    = esc_html( '%1$s installed successfully.', 'tgmpa' ) . ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html( 'Show Details', 'tgmpa' ) . '</span><span class="hidden">' . esc_html( 'Hide Details', 'tgmpa' ) . '</span>.</a>';
					...
					public function bulk_footer() {
						...
						if ( $this->tgmpa->is_tgmpa_complete() ) {
							...
							$update_actions['dashboard'] = sprintf(
								...
								'<a href="' . esc_url( self_admin_url() ) . '">' . esc_html( 'Return to the Dashboard', 'tgmpa' ) . '</a>'
							);
						} else {
